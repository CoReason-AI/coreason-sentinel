...
class CircuitBreaker:
    ...
    async def _record_metric_internal(
        ...
        if user_context and user_context.user_id:
            key = f"sentinel:metrics:{self.agent_id}:{user_context.user_id}:{metric_name}"
        else:
    ...
    async def _check_triggers_internal(self, user_context: Optional[UserContext]) -> None:
        ...
        for trigger in self.config.triggers:
            if await self._evaluate_trigger(trigger, now, user_context):
                ...
                prefix = f"User {user_context.user_id}: " if user_context and user_context.user_id else "Global: "
    ...
    async def _evaluate_trigger(
        ...
        """
        if user_context and user_context.user_id:
            key = f"sentinel:metrics:{self.agent_id}:{user_context.user_id}:{trigger.metric}"
        else:
