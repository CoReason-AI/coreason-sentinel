...
class TelemetryIngestorAsync:
    ...
    async def process_otel_span(self, span: OTELSpan) -> None:
        """
        ...
        if span.end_time_unix_nano > span.start_time_unix_nano:
            ...
            await self.circuit_breaker.record_metric("latency", latency_sec)
        ...
        if token_count > 0:
            await self.circuit_breaker.record_metric("token_count", token_count)

            if self.config.cost_per_1k_tokens > 0:
                cost = (token_count / 1000.0) * self.config.cost_per_1k_tokens
                await self.circuit_breaker.record_metric("cost", cost)
    ...
    async def process_event(self, event: VeritasEvent) -> None:
        """
        ...
        for metric_name, value in event.metrics.items():
            if isinstance(value, (int, float)):
                await self.circuit_breaker.record_metric(metric_name, float(value))
        ...
        if self.spot_checker.should_sample(combined_metadata):
            ...
            if grade:
                await self.circuit_breaker.record_metric("faithfulness_score", grade.faithfulness_score)
                await self.circuit_breaker.record_metric("retrieval_precision_score", grade.retrieval_precision_score)
                await self.circuit_breaker.record_metric("safety_score", grade.safety_score)

        await self.circuit_breaker.check_triggers()

    async def process_drift(self, event: VeritasEvent) -> None:
        """
        ...
        if embedding and isinstance(embedding, list):
            try:
                ...
                baselines = await anyio.to_thread.run_sync(self.baseline_provider.get_baseline_vectors, event.agent_id)
                if baselines:
                    # Calculation -> CPU bound
                    drift_score = await anyio.to_thread.run_sync(
                        DriftEngine.detect_vector_drift, baselines, [embedding]
                    )
                    await self.circuit_breaker.record_metric("vector_drift", drift_score)
            except Exception as e:
                logger.error(f"Failed to process vector drift detection: {e}")

        # 2. Drift Detection (Output Length)
        try:
            await self._process_output_drift(event)
        except Exception as e:
        ...
        if (
            ...
            try:
                ...
                await self.circuit_breaker.record_metric("relevance_drift", relevance_drift)
            except Exception as e:
    ...
    async def _process_output_drift(self, event: VeritasEvent) -> None:
        """
        ...
        await self.circuit_breaker.record_metric("output_length", output_length)

        try:
            # DB call
            baseline_dist, bin_edges = await anyio.to_thread.run_sync(
                self.baseline_provider.get_baseline_output_length_distribution, event.agent_id
            )
        ...
        recent_samples = await self.circuit_breaker.get_recent_values("output_length", self.config.drift_sample_window)
        ...
        try:
            ...
            await self.circuit_breaker.record_metric("output_drift_kl", kl_divergence)
...
class TelemetryIngestor:
    ...
    def process_otel_span(self, span: OTELSpan) -> None:
        if not self._portal:
            anyio.run(self._async.process_otel_span, span)  # pragma: no cover
        else:
            self._portal.call(self._async.process_otel_span, span)

    def ingest_from_veritas_since(self, since: datetime) -> int:
        if not self._portal:
            return cast(int, anyio.run(self._async.ingest_from_veritas_since, since))  # pragma: no cover
        else:
    ...
    def process_event(self, event: VeritasEvent) -> None:
        if not self._portal:
            anyio.run(self._async.process_event, event)  # pragma: no cover
        else:
            self._portal.call(self._async.process_event, event)

    def process_drift(self, event: VeritasEvent) -> None:
        if not self._portal:
            anyio.run(self._async.process_drift, event)  # pragma: no cover
        else:
